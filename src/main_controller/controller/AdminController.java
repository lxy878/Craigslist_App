...
public class AdminController {
    @FXML BorderPane admin_main;
    @FXML Button button_return, button_search, button_rebuild;
    @FXML CheckBox cb_offline;
    @FXML ComboBox<String> city_list, search_type;
    @FXML TextField target, limit, target_users;
    @FXML ListView<String> list_result;
    ...
    @FXML public void buttonAction(ActionEvent event) throws Exception{
        Stage currentStage = (Stage)admin_main.getScene().getWindow();
        Parent temp;
        if(event.getSource().equals(button_return)){
            ...
            listCheck();
        }else if(event.getSource().equals(button_rebuild)){
            rebuild();
        }
    }
    ...
    @FXML public void checkBox(){
        boolean disable = cb_offline.isSelected();
        target_users.setDisable(!disable);
        limit.setDisable(disable);
        search_type.setDisable(disable);
    }

    @FXML private void rebuild(){
        userp.rebuildData();
    }
    ...
    private void online(){
        String input = "serverCity="+city_list.getValue()+";searchType="
                +search_type.getValue()+";search="+target.getText()+";limit="+limit.getText();
    ...
    private void offline(){
        ...
        String account = target_users.getText();

        List<String> titleList = userp.offline(url, search, account);
    ...
    private void setImage(String id){
        ...
        if(imagePath.length()==0){
            imagePath = FileProcessClass.image_path+"no_image.png";
        }
